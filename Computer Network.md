...
4. 如果证书受信任，或者用户接收了不受信任的证书，浏览器会生成一个**随机密钥**（用于对称算法），并用服务器提供的公钥加密（采用非对称算法对密钥加密）；使用Hash算法对握手消息进行**摘要**计算，并对握手信息和握手信息的摘要使用之前产生的密钥加密（对称算法）；将加密后的随机密钥和加密后的摘要一起发送给服务器；(其中计算握手信息的摘要，目的是为了保证传回到服务端的握手信息没有被篡改。)
