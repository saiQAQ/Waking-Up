...
4. 如果证书受信任，或者用户接收了不受信任的证书，浏览器会生成一个**随机密钥**（用于对称算法），并用服务器提供的公钥加密（采用非对称算法对密钥加密）；使用Hash算法对握手消息进行**摘要**计算，并对握手信息和相应的摘要使用之前产生的**随机密钥**加密（对称算法）；将加密后的随机密钥和用随机密钥加密的摘要一起发送给服务器；(其中计算握手信息的摘要，目的是为了保证传回到服务端的握手信息没有被篡改。)
5. 服务器使用自己的私钥解密，得到对称加密的密钥，用这个密钥解密出Hash摘要值，同时计算自己发送的握手信息的摘要值，与客户端传回来的进行对比验证。如果一致，服务器使用对称加密的密钥加密握手消息和摘要值发给浏览器；
6. 浏览器解密并验证摘要，若一致，则完成握手。之后的数据传送都使用对称加密的密钥进行加密。

附:https://blog.csdn.net/qq_24601199/article/details/104362401  帮助理解

总结：非对称加密算法用于在握手过程中加密随机生成的密钥；对称加密算法用于对真正传输的数据进行加密；HASH算法用于验证传输数据的完整性。
</details>
