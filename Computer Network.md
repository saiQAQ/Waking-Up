...
使用滑动窗口协议实现流量控制。防止发送方发送速率太快，接收方缓存区不够导致溢出。接收方会维护一个接收窗口 receiver window（窗口大小单位是字节），接受窗口的大小是根据自己的资源情况动态调整的，在返回ACK时将接受窗口大小放在TCP报文中的窗口字段告知发送方。发送窗口的大小不能超过接受窗口的大小，只有当发送方发送并收到确认之后，才能将发送窗口右移。下面这张图表示发送窗口：
...
3. 快重传：快重传要求接收方在没有收到自己期望的报文段时就立即发出**重复确认**（为的是使发送方及早知道有报文段没有到达对方）而不要等到自己发送数据时捎带确认。快重传算法规定，发送方只要一连收到三个重复确认就应当立即重传对方尚未收到的报文段，而不必继续等待设置的重传计时器时间到期。
...
2. 服务器从中选出一组自己也支持的加密算法与HASH算法，并将自己的身份信息以证书的形式发回给浏览器。证书里面包含了网站地址，**公钥**（用于非对称加密），以及证书的颁发机构等信息。
3. 客户端验证证书的有效性，包括：证书是否过期，CA 是否可靠，发行者证书的公钥能否正确解开服务器证书的“发行者的数字签名”，服务器证书上的域名是否和服务器的实际域名相匹配；
4. 客户端验证证书无误后（或者接受了不信任的证书），浏览器会生成一个**随机密钥**（用于对称算法），并用服务器提供的公钥进行加密（采用非对称算法对密钥加密）；然后使用Hash算法计算握手消息的**摘要**，然后用生成的随机数将握手信息和相应的摘要进行加密，再用公钥将随机数进行加密；最后将加密后的随机数和用随机数加密的摘要一起发送给服务器；(其中计算握手信息的摘要，目的是为了保证传回到服务端的握手信息没有被篡改。) 
5. 服务器使用自己的私钥解密，得到对称加密的随机数，用这个随机数解密出摘要值，同时自己也计算一遍的握手信息的摘要值，与客户端传回来的进行对比验证。如果一致，服务器使用对称加密的随机数加密握手消息和摘要值发给浏览器；
6. 浏览器解密并再一次验证摘要，若一致，则完成握手。从此服务端和客户端就开始用那串随机数进行对称加密通信了。

附:https://blog.csdn.net/qq_24601199/article/details/104362401  帮助理解

**简化版本**（看这个好理解）：
1. 客户端发起https连接  
2. 服务端发送证书  
3. 客户端验证服务端发来的证书：验证证书, 生成随机数（此随机数就是后面用的对称加密的私钥）, 生成握手信息的摘要。公钥包住随机数再包住摘要, 发给服务端。
4. 服务端用私钥解开公钥得到随机数, 再用随机数解密得到摘要，同时自己算一遍摘要，对比看看客户端传过来的过程种握手信息是否被篡改。一致，说明客户端发送过来是安全的。再用随机数加密摘要发回给客户端。
5. 客户端用随机数解密，然后验证服务端发送回来的摘要，无误说明服务端发送过来也是安全的，完成握手。
   
总结：非对称加密用于在握手过程中加密随机生成的密钥；对称加密算法用于对真正传输的数据进行加密；HASH算法计算的摘要用于验证传输数据的完整性。
</details>
...
[你访问的网站是如何自动切换到 HTTPS 的？](https://www.sohu.com/a/136637876_487516)
   ...
   **简单来说就是, 要访问的服务端站点它引入了HSTS机制, 这是一种网站用来声明他们只能接受HTTPS访问的方法, HSTS可以强制访问它的浏览器使用 HTTPS 协议。 以最大限度的减少 HTTP 协议的各种安全问题了。**

（最原始的方法是302跳转，服务器把所有的HTTP流量跳转到HTTPS。但这样有一个漏洞，就是中间人可能在第一次访问站点的时候就劫持。
解决方法是在服务端引入HSTS机制，用户浏览器在访问站点的时候强制使用HTTPS。）
</details>
...
- 非对称加密：需要两个密钥：公钥和私钥。如果用公钥加密，需要用私钥才能解密。如：RSA. 注意: 私钥加密的密文只有公钥才能解开；公钥加密的密文只有私钥才能解开。
...
- 关于数字签名，我自己也查了很多资料，我是这样理解的，签名就是通俗理解就是自己证明自己，用私钥对报文的摘要进行签名（即加密），公钥验签（解密）。  
   发送者：
   节点A 将报文 HASH ,然后生成一个报文摘要。
   ...
   在HTTPs握手中体现在：用CA的公钥能否正确解开服务器证书的“发行者的数字签名”，可以的话就说明有效。（数字签名被数字证书用私钥加密）  
   发送者A用私钥进行签名，接收者B用公钥验证签名。因为除A外没有人有私钥，所以B相信签名是来自A。A不可抵赖，B也不能伪造报文。

- 常用的摘要算法:MD5、SHA
  
附录帮助理解：  
   https://blog.csdn.net/jlin991/article/details/59110060  
   https://blog.csdn.net/qq_24601199/article/details/104338962  
   https://blog.csdn.net/u010033948/article/details/62881334?locationNum=4&fps=1  
   
</details>

### GET与POST的区别？

1. GET是幂等的，即读取同一个资源，总是得到相同的数据，POST不是幂等的； (从定义上看，HTTP方法的幂等性是指一次和多次请求某一个资源应该具有同样的作用。说白了就是，同一个请求，发送一次和发送N次效果是一样的！)
2. GET一般用于从服务器获取资源，而POST有可能改变服务器上的资源；
3. 请求形式上：GET请求的数据附在URL之后，在HTTP请求头中；POST提交数据,请求服务器进行处理,提交的数据在请求体中；
...
Session保存在服务端，客户端浏览器再次访问的时候通过检索Session id来查看该客户的状态。保存Session id的方式可以采用Cookie，如果禁用了Cookie，可以使用URL重写机制（把Session id重写在URL中，服务器能够解析重写后的URL获取Session的id）。

附录：https://www.cnblogs.com/l199616j/p/11195667.html 写的很好，很容易理解

### 从输入网址到获得页面的过程 (越详细越好)？

1. 浏览器查询 DNS，获取域名对应的IP地址，其顺序是：浏览器缓存，操作系统缓存，路由器缓存，本地DNS服务器，根域名服务器缓存，顶级域名服务器缓存，主域名服务器缓存。对于向本地DNS服务器进行查询，如果要查询的域名由本地DNS服务器负责解析，则返回解析结果给客户机，完成域名解析(此解析具有权威性)；如果要查询的域名是不由本地DNS服务器负责解析，但该服务器缓存了此网址的映射关系，则调用这个IP地址映射，完成域名解析（此解析不具有权威性）。如果本地域名服务器并未缓存该网址映射关系，那么将根据其设置发起递归查询或者迭代查询；
2. 浏览器获得域名对应的IP地址以后，浏览器向服务器请求建立连接，发起TCP三次握手；
3. TCP/IP连接建立起来后，浏览器向服务器发送HTTP请求；
4. 服务器的永久重定向响应（从 http://example.com 到 http://www.example.com ），浏览器跟踪重定向地址。
